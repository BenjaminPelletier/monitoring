    ...
    for config_name, report_path in zip(config_names, report_paths):
        ...
        if config_name != config_in_report:
            whole_config = load_config(
                config_name, skip_validation=args.skip_validation
            )
        else:
            whole_config = report.configuration

        if "artifacts" in whole_config and whole_config.artifacts:
            logger.debug("Generating artifacts...")
            if args.output_path:
                output_path = args.output_path
            else:
                output_path = str(Path(resolve_filename(report_path)).parent)
            generate_artifacts(whole_config.artifacts, report, output_path)
        else:
            output_path = "nowhere"
