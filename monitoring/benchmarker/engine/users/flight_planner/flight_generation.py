...
class IndependentTimeLocationShape(FlightGenerator):
    ...
    def __init__(
        ...
        if (
            ...
            "uniform_random_location" in self.spec.location
            and self.spec.location.uniform_random_location
        ):
            pass
        else:
        ...
        if self.spec.shape.fixed_volumes:
            ...
            if (
                ...
                loc_vert = self.spec.location.uniform_random_location.vertical
            else:
                loc_vert = None
            if loc_vert and (
    ...
    def generate_flight(self, previous_flight_end: datetime) -> Flight:
        ...
        if "fixed_location" in self.spec.location and self.spec.location.fixed_location:
            ...
            box = self.spec.location.uniform_random_location.horizontal
            xy = LatLngPoint(
                lat=self.random.uniform(box.lat_min, box.lat_max),
                lng=self.random.uniform(box.lng_min, box.lng_max),
            )
            z = self.spec.location.uniform_random_location.vertical
            if z.units != DistanceUnits.M:
                raise NotImplementedError(
                    "Only meters are supported for uniform_random_location altitude"
                )
            if z.reference != AltitudeDatum.W84:
                raise NotImplementedError(
                    "Only W84 is supported for uniform_random_location altitude"
                )
        else:
