...
* **Package-based**: refer to a dictionary (*.json, *.yaml) file located in a subfolder of the `uss_qualifier` folder using the Python module style, omitting the extension of the file name.  For instance, `configurations.dev.all_local_tests` would refer to [uss_qualifier/configurations/dev/all_local_tests.yaml](dev/all_local_tests.yaml).
